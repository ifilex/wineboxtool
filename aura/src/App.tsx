...
export function App() {
    ...
    // ============================================================
    // ESTADOS PARA ANIMACIÓN DE CARGA (SIN PORCENTAJE VISIBLE)
    // ============================================================
    ...
    // ============================================================
    // FRASES DE CARGA (solo texto amigable)
    // ============================================================
    ...
    // ============================================================
    // SIMULAR PROGRESO DE CARGA (SOLO PARA LA ANIMACIÓN)
    // ============================================================
    useEffect(() => {
        if (!loadFinished && !criticalError && showSplash) {
            ...
            const phraseInterval = setInterval(() => {
                ...
                setLoadingText(loadingPhrases[loadingStage]);
            }, 2500);
    ...
    }, [loadFinished, criticalError, loadingStage, showSplash]);

    // ============================================================
    // CUANDO TERMINA LA CARGA, COMPLETAR PROGRESO Y OCULTAR SPLASH
    // ============================================================
    useEffect(() => {
        if (loadFinished) {
    ...
    useEffect(() => {
        const initApp = async () => {
            try {
                const gpuResult = await isWebGPUok();
                if (gpuResult !== true) {
                    dispatch(setCriticalError('WebGPU error: ' + gpuResult));
                    return;
                }
                ...
                if (navigator.storage && navigator.storage.estimate) {
                    ...
                    if (estimate) {
                        ...
                        if (!alreadyFromCache && remainingMb > 10 && remainingMb < MODEL_SIZE_MB) {
                            dispatch(setCriticalError('Espacio de almacenamiento insuficiente'));
                        }
                    }
                }
                ...
                dispatch(setCriticalError('Error al inicializar: ' + error));
            }
        };
    ...
    // ============================================================
    // RENDER - PANTALLA DE CARGA (SIN PORCENTAJE VISIBLE)
    // ============================================================
    if (showSplash && !loadFinished && !criticalError) {
        return (
            <ThemeProvider theme={darkTheme}>
                ...
                <Box sx={{
                        ...
                        {[0, 1, 2, 3, 4, 5].map((i) => (
                            <Box
                                ...
                                sx={{
                                    ...
                                    height: 8,
                                    borderRadius: '50%',
                                    bgcolor: `rgba(144, 202, 249, ${0.3 + Math.random() * 0.5})`,
                                    top: `${50 + 70 * Math.cos(i * Math.PI / 3)}%`,
                                    left: `${50 + 70 * Math.sin(i * Math.PI / 3)}%`,
                                    transform: 'translate(-50%, -50%)',
                    ...
                    {/* Texto de carga - AMIGABLE, SIN TÉCNICA */}
                    <Typography
                        variant="h5"
                        sx={{
                            fontWeight: 300,
                    ...
                    {/* Botón de descarga (solo si no hay modelo) - SIN TÉCNICA */}
                    {!alreadyFromCache && !localStorage.getItem('downloaded_models') && (
                        <Button
                            variant="outlined"
                            onClick={() => {
                                setLoadingText('🌱 Descargando tu compañera...');
                                downloadModel(MODEL).then(() => {
                                    setLoadFinished(true);
                                });
                            }}
                            sx={{
                    ...
                    <Box sx={{ display: 'flex', alignItems: 'center', gap: 1.5 }}>
                        ...
                        <Box sx={{ 
                            ...
                            <Box sx={{
                                ...
                                boxShadow: loadFinished ? '0 0 10px #4ade80' : '0 0 10px #ffd700',
                                animation: 'pulse 2s ease-in-out infinite'
                            }} />
                        </Box>
                        <Box>
                            ...
                            <Typography variant="caption" sx={{ 
                                ...
                                {loadFinished ? 'En línea' : 'Cargando...'}
                            </Typography>
                        </Box>
                    </Box>
